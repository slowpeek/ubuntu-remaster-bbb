...
Supported iso versions: =14.04= to =26.04= for desktop, =18.04= to =26.04= for
live-server.
...
f2fs and exfat modules (as of Q2 2026), so we have to make grub NOT use the
...
itself does not include f2fs into its list of supported filesystems (as of Q2
2026):
